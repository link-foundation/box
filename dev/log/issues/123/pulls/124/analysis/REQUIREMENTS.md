...
| `apt-get update` inheriting a retry count instead of passing one | 3 refresh sites, 137 sources swept | `test-issue123-apt-retry-defaults.sh` part 4, over **logical** lines — all three real sites spell the option on a `\`-continuation |
