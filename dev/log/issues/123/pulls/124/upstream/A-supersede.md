...
`run_is_superseded` compares `RUN_SHA` with the branch head (`__SUPERSEDE_LINE__`). Nothing in it, or around it, looks at the cancelled job.
...
This is a follow-up to __PRIOR__, which added the gate and the supersede test. That issue was right that a cancellation on `main` needs a second question before it can be excused; this one is that the second question has to be asked of the job, not of the run.
