...
Follow-up to __PRIOR__. That fix moved liveness from `wait` to a poll on the process group and is a genuine improvement; what it does not cover is a command whose root *exits* while its children continue, and the output inheritance that turns any survivor into a hung step.
