...
`scripts/check-pipeline-status.sh:80-89` @ `15c327be`
...
`run_is_superseded` compares `RUN_SHA` with the branch head (`line 47`). Nothing in it, or around it, looks at the cancelled job.
...
Measured against `php` at `15c327be682deab6b65c69d48ef43e069d60bd01`:

```
### Case A: this run is still the head of main
Failed jobs:    <none>
Cancelled jobs: measure
::error::Pipeline has cancelled jobs on main: measure. This run was still the head of main when the jobs were cancelled, so the cancellation is a real failure -- most likely a job killed by 'timeout-minutes', which GitHub reports as cancelled. Steps wrapped in scripts/run-with-budget-warning.sh report a real timeout as a failure instead, which is why long steps carry their own budget.
exit=1

### Case B: a later commit landed on main while the job was timing out
Failed jobs:    <none>
Cancelled jobs: measure
::warning::Cancelled jobs: measure. This run is no longer the head of main: a newer commit superseded it, so the cancellation is expected churn.
exit=0
```
...
This is a follow-up to [php#9](https://github.com/link-foundation/php-ai-driven-development-pipeline-template/issues/9), which added the gate and the supersede test. That issue was right that a cancellation on `main` needs a second question before it can be excused; this one is that the second question has to be asked of the job, not of the run.
...
<details>
<summary><code>experiments/issue-123/repro-supersede.sh</code></summary>
