...
`scripts/check-pipeline-status.sh:69-78` @ `470e1760`
...
`run_is_superseded` compares `RUN_SHA` with the branch head (`line 36`). Nothing in it, or around it, looks at the cancelled job.
...
This is a follow-up to [python#69](https://github.com/link-foundation/python-ai-driven-development-pipeline-template/issues/69), which added the gate and the supersede test. That issue was right that a cancellation on `main` needs a second question before it can be excused; this one is that the second question has to be asked of the job, not of the run.
