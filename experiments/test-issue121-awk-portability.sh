...
CONTRACT_GATE="scripts/ci/check-awk-portability.sh"
CONTRACT_RC=0
...
printf '%s\n' "$CONTRACT_OUT" | grep -qx -- 'scripts/ci/check-awk-portability.sh' \
  && pass "and names scripts/ci/check-awk-portability.sh, which this gate demonstrably reads" \
  || fail "--list-inputs omits scripts/ci/check-awk-portability.sh"
